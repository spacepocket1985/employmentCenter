...
// Валидация сложного времени
const timeValueSchema = yup.object().shape({
  type: yup.string().oneOf(['simple', 'range', 'text', 'continued']).required(),
  simpleTime: yup.string().when('type', {
    is: 'simple',
    then: (schema) => schema.required('Время обязательно').matches(
  ...
  dayRange: yup.object().when('type', {
    ...
    then: (schema) => schema.shape({
      ...
      time: yup.string().required().matches(
        /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/,
        'Формат ЧЧ:ММ'
      ),
    }),
  ...
  daySpecific: yup.object().when('type', {
    is: 'daySpecific',
    then: (schema) => schema.shape({
      monday_thursday: yup.string().matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Формат ЧЧ:ММ'),
      friday: yup.string().matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Формат ЧЧ:ММ'),
      saturday: yup.string().matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Формат ЧЧ:ММ'),
      sunday: yup.string().matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Формат ЧЧ:ММ'),
      working: yup.string().matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Формат ЧЧ:ММ'),
      weekend: yup.string().matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Формат ЧЧ:ММ'),
    }),
    otherwise: (schema) => schema.notRequired(),
  }),
...
const busStopSchema = yup.object().shape({
  ...
  address: yup
    .string()
    .required('Адрес остановки обязателен')
...
const dayTypeSchema = yup.string().oneOf([
  'working', 'weekend', 'holiday', 
  'monday_thursday', 'friday', 'saturday', 'sunday'
]);
...
const scheduleEntrySchema = yup.object().shape({
  ...
  dayTypes: yup.array().of(dayTypeSchema).min(1, 'Выберите хотя бы один тип дня'),
