import { TFoodMenuDayResponse, IDish } from 'src/types/foodMenu.types';
import { CSVValidationResult } from '../types/menu.types';
...
function parseCSVRows(rows: string[][]): CSVValidationResult {
  const errors: string[] = [];
  const menu: TFoodMenuDayResponse [] = [];
  
  let currentDayMenu: TFoodMenuDayResponse | null = null;
  let dishCounter = 0;
